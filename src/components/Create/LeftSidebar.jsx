...
function LeftSidebar() {
  const { setMainQuestion, mainQuestion, displayQuestion, setDisplayQuestion, quiz, setQuiz } = useContext(Question)
    ...
    if (questionIndex === 1) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          setMainQuestion((prevQuestions) => prevQuestions.filter((q) => q.questionIndex !== questionIndex))
  ...
  return (
    <div className="sidebar">
      <input type="text" placeholder="Quiz Name" onChange={handleQuizName} value={quiz.name || ""} />
        ...
        >
          <div className="quiz">
            <div className="quiz-name">
              <div className="name">{`Quiz ${i + 1}`}</div>
      ...
      <div className="add-button">
        <button onClick={addQuestion} className="btn" style={{ border: "1px solid black" }}>
          Add Question
        </button>
      </div>
    </div>
